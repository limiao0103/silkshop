...
;(function($){
	$.extend($.fn,{
		shoping:function(options){
			var self=this,
				$shop=$('.J-shoping'),//购物车外层的盒子
				$title=$('.J-shoping-title'),//购物车数量外层盒子
				$body=$('.J-shoping-body'),//加入购物车的商品列表
				$num=$('.J-shoping-num'),//商品数量
			...
			var S={
				...
				addShoping:function(e){
					...
					var $target=$(e.target),
						id=$target.attr('id'),
						src=$target.attr('productsrc'),
                        tit=$target.attr('producttit'),
					...
					if(dis){
						if ($('#floatOrder').length <= 0) {
							$('body').append('<div id="floatOrder"><image src="'+src+'" width="50" height="50" /></div>');
						};
						var $obj=$('#floatOrder');
						if(!$obj.is(':animated')){
							$obj.css({'left': x,'top': y}).animate({'left': X,'top': Y-80},0,function() {
								$obj.stop(false, false).animate({'top': Y-20,'opacity':0},0,function(){
									$obj.fadeOut(300,function(){
										$obj.remove();	
										$target.data('click',false).addClass('dis-click');
                                        $target.data('click',false).siblings('span').html("成功加入询盘车").fadeIn(300).delay(500).fadeOut(300);
										var l=$('.J-shoping-list').length,
											num=Number($num.text());
                                        $body.prepend('<div class="J-shoping-list" data-id="'+id+'"><a href="#"title=""><image src="'+src+'" width="50"height="50"/></a><div class="J-shoping-list-a"><p>'+tit+'</p></div><div class="baseBg J-shoping-close"></div></div>');
										$num.text(num+1);
									});
								});
							});	
				...
				clickOnTitle:function(e){
					...
					if(length>0){
						if(!$shop.hasClass('J-shoping-small')){
							...
							$('.J-shoping-mx').hide();
							$('.J-shoping-px').show();
							$shop.animate({'width':148},100,function(){
